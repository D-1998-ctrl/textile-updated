...
function Liveorder() {
  ...
  const handleCancelOrderClick = (order) => {
    ...
    fetch("https://textileapp.microtechsolutions.co.in/php/updateloomorder.php?LoomOrderId="+order+"&Confirmed=false", requestOptions)
      .then((response) => response.text())
      .then((result) => {
        //               //console.log(result);
        toast.success(`Cancelled OR${order}`);
        loadliveorders();
      })
      .catch((error) => console.error(error));
  }
